...
void Start_Testing(void)
{
	...
	if(TestStep.Test_Start == 1)
	{
		...
		{			
			...
			case Starting_Up:
			{
				if(htRxdata_P.WaitTimerFlag == 0)  //ÉÏµçµÈ2S ·¢ËÍ×Ô¶¯Ñ§Ï°Ö¸Áî
				{
					Test_Relay_L;
					DataSwitch_Relay_H;			//Êý¾Ý½ÅÁ¬½Ó	
					htRxdata_P.WaitTimerFlag =1;
				}
			...
			case HW_Learn:				 //  ·¢ËÍ×Ô¶¯Ñ§Ï°Ö¸Áî  µÈ´ý5S ²é¿´ÊÇ·ñ·µ»ØÊý¾Ý
			{				
				...
				switch(GanYing_Com)
				{
					...
					case 1:
					{
						...
						else if(htRxdata_P.WaitTimerFlag == 2)   //×ÔÑ§Ï°Ê§°Ü .¡£¡£¡£»¹Î´ÐÞ¸Ä
						{
							TestStep.Test_Press = Ganying_Data;	
							htRxdata_P.WaitTimerFlag = 0;
							ComP.Com_Flag = 1;//ÏÂÒ»¸ö¶ÁÈ¡¿ªÊ¼
			...
			case Ganying_Data:
			{
				...
				if(TestPara.IndicLigh_Current_Flag == 1)  //Ö¸Ê¾µÆµçÁ÷¶ÁÈ¡
				{
					Com_Send(&TestPara.Light_Current_Test);  //200ms×Ô¶¯ÍË³ö
					if(ComP.Com_Flag ==0)
					{
						ComP.Com_Flag = 1;//ÏÂÒ»¸ö¶ÁÈ¡¿ªÊ¼
						ComP.Com_Step = 0;
				...
				else if(TestPara.Power_Current_Flag == 1)  //Çý¶¯µçÁ÷¶ÁÈ¡
				{
					Com_Send(&TestPara.Power_Current_Test);  //200ms×Ô¶¯ÍË³ö
					if(ComP.Com_Flag ==0)
					{
						ComP.Com_Flag = 1;//ÏÂÒ»¸ö¶ÁÈ¡¿ªÊ¼
						ComP.Com_Step = 0;
				...
				else if(TestPara.Ganying_Data_Flag==1)   //¸ÐÓ¦Êý¾Ý¶ÁÈ¡
				{
				...
				else if(htRxdata_P.WaitTimerFlag == 2)  //ÏÂÒ»¸ö¼ì²â
				{
						//TestStep.Test_Press = Standby_Current;
						htRxdata_P.WaitTimerFlag = 0;
						htRxdata_P.WaitTimer = 0;
			...
			case Standby_Current:
			{
				...
				if(htRxdata_P.WaitTimerFlag == 2)
						Com_Send(&TestPara.Standby_Current_Test);  //200ms×Ô¶¯ÍË³ö
				if(ComP.Com_Flag == 0)
				{
			...
			{	
				...
				if(htRxdata_P.WaitTimerFlag == 2)
				{
					switch(TestStep.Motor_Step)
					{
						...
						{
							Motor_DirP;
							for(StepN_Num=0;StepN_Num<10270;StepN_Num++) // 790¿É¸ü¸ÄÎª¾ßÌåµÄ²ÎÊýÖµ(´Ó¶¥²¿µ½¾àÀëÎª6)
							{
								Motor_PulP;
								delay_us(50);
								Motor_PulN;
							...
							if(GATE_Flag== 0)					//Ìø³ö¼ì²âÌáÊ¾ÎªµÍ
							{
						...
						{
							Motor_DirN;
							for(StepN_Num=0;StepN_Num<1580;StepN_Num++) // 790¿É¸ü¸ÄÎª¾ßÌåµÄ²ÎÊýÖµ(´Ó¾àÀëÎª6->¾àÀëÎª8)
							{
								Motor_PulP;
								delay_us(50);
								Motor_PulN;
								delay_us(50);
									if(GPIO_ReadInputDataBit(GPIOE, GATE1_DET) == 0) //¼ì²âµ½ Ôò¿ªÊ¼¼ì²âÉÏÏßÖµ
									{
										GATE_Flag = 2;
									}
							}	
...
void Data3_Send(void)
{
	...
	Data_Deal_Send(Para_Data.HW_Learn_Max,0X0003,ReadTextControl_id[8]);			//ºìÍâ·¢Éä¹¦ÂÊ
	Data_Deal_Send(Para_Data.HW_Learn_Min,0X0003,ReadTextControl_id[9]);
	Data_Deal_Send(Para_Data.Shoot_Power_Max ,0X0003,ReadTextControl_id[10]);				//ºìÍâÑ§Ï°½á¹û
